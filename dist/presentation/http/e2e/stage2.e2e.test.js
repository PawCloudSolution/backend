...
describe('Stage 2 E2E Flow (Employees and Branches)', () => {
    ...
    it('1. setup: create superadmin and HQ', async () => {
        ...
        if (resLogin.status !== 201)
            throw new Error('SA Login failed: ' + JSON.stringify(resLogin.body));
        ...
        if (resIntSubmit.status !== 201)
            throw new Error('Int submit failed: ' + JSON.stringify(resIntSubmit.body));
        ...
        if (resApp.status !== 201)
            throw new Error('HQ submit failed: ' + JSON.stringify(resApp.body));
        ...
        if (resApprove.status !== 200 && resApprove.status !== 201)
            throw new Error('HQ approve failed: ' + JSON.stringify(resApprove.body));
        // 1.5 Login president to get org ID
        const resPresLogin = await request(app.getHttpServer()).post('/api/v1/auth/login').send({
            email: 'hq@example.com',
            password: 'hqpass'
        });
        if (resPresLogin.status !== 201)
            throw new Error('Pres Login failed: ' + JSON.stringify(resPresLogin.body));
