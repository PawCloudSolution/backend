...
describe('Stage 3 Flow (E2E) - Dogs and Breeds', () => {
    ...
    let clubOrgId;
    let breedId;
    let breedApplicationId;
    let ownerId;
    let breederId;
    let superAdminCookies;
    let hqPresidentCookies;
    let clubPresidentCookies;
    beforeAll(async () => {
        ...
        if (saRes.status !== 201)
            throw new Error('SA register failed: ' + JSON.stringify(saRes.body));
        ...
        if (intAppRes.status !== 201)
            throw new Error('Int submit failed: ' + JSON.stringify(intAppRes.body));
        ...
        if (hqAppRes.status !== 201)
            throw new Error('HQ submit failed: ' + JSON.stringify(hqAppRes.body));
        ...
        if (hqApproveRes.status !== 200 && hqApproveRes.status !== 201)
            throw new Error('HQ approve failed: ' + JSON.stringify(hqApproveRes.body));
        ...
        if (clubRes.status !== 201)
            throw new Error('Club create failed: ' + JSON.stringify(clubRes.body));
        ...
        if (ownerRes.status !== 201)
            throw new Error('Owner reg failed: ' + JSON.stringify(ownerRes.body));
        ...
        if (breederRes.status !== 201)
            throw new Error('Breeder reg failed: ' + JSON.stringify(breederRes.body));
    ...
    it('1. Club President submits a Breed Application', async () => {
        ...
        if (res.status !== 201)
            console.log('Test 1 error:', res.body);
    ...
    it('3. Anyone can fetch the list of breeds', async () => {
        ...
        const breed = res.body.find((b) => b.id === breedId);
